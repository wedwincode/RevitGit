...
The Revit coordinator does not save the dirty document and does not close the active document directly. It activates the staged family, closes the now-inactive original with `Close(false)`, atomically publishes the staged bytes at the original path, finalizes history, then opens and activates the original path again. Intermediate document events are suppressed while restore is active. If finalize fails after replacement, the coordinator republishes the verified prior current version and attempts to reopen it. If automatic reopen fails after successful finalize, disk content and history remain consistent and the UI reports the original path for manual reopening.
